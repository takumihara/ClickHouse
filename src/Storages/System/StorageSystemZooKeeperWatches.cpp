...
ColumnsDescription StorageSystemZooKeeperWatches::getColumnsDescription()
{
    ...
    return ColumnsDescription{
        ...
        {"create_time_microseconds", std::make_shared<DataTypeDateTime64>(6), "Time when the watch was created with microsecond precision."},
    ...
    MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    auto fill = [&](const std::string_view name, const zkutil::ZooKeeperPtr & zookeeper)
    {
        ...
        for (const auto & entry : tracker->getSnapshot())
        {
            auto create_ts       = entry->create_time.time_since_epoch();
            ...
            res_columns[3]->insert(entry->path);
            res_columns[4]->insert(zookeeper->getClientID());
            res_columns[5]->insert(entry->request_xid);
            res_columns[6]->insert(static_cast<Int16>(entry->op_num));
            res_columns[7]->insert(watchTypeFromOpNum(entry->op_num));
        }
    };
